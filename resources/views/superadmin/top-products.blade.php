...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Classement global du {{ $dateFrom->format('d/m/Y') }} au {{ $dateTo->format('d/m/Y') }}.</p>
        </div>
        <a href="{{ route('superadmin.sales') }}" class="inline-flex w-fit items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-bold text-gray-700 shadow-sm hover:bg-gray-50">
            Retour aux ventes
        </a>
    </div>

    <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('superadmin.sales.top-products') }}" class="grid gap-4 sm:grid-cols-[1fr_1fr_1fr_auto] sm:items-end">
            <div>
        ...
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Gerant</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse($products as $product)
                    <tr>
                        ...
                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">{{ $product->manager_name ?? '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-3 py-8 text-center text-sm text-gray-500">Aucun produit vendu sur cette periode.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
